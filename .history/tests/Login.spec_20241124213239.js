...
test('Login to the Portal and login as Data Entry A', async ({ page }) => {
   // Visit the portal
   await page.goto('https://qa.akyrian.com/', { timeout: 30000 });
   await page.waitForTimeout(10000);
...
await page.locator('[data-cy="visit-cm31qz0990dhkigpbpkpgy2p5"]').filter({ hasText: 'AUTOMATION-sign-CRF' }).click();
    ...
    await page.waitForTimeout(10000);
  // Locate and click action menu icon
  const actionMenuIconLocator = page.locator('[data-cy="data-entry-action-cm2n696qj08n8zdk3hgiru340"]', { force: true }).first();
  await actionMenuIconLocator.click();
    // Type "allergen" in the answer field
    await page.fill('[data-cy="answer-input-field-cm2n696mo00000t5m8a76d1vb-0-0"]', 'allergen');

    // Submit Data Entry
    await page.click('[data-cy="submit-data-entry"]');
    
    // Check Data Entry - Blood Pressure
    await page.waitForTimeout(10000);
    await page.fill('[data-cy="answer-input-field-cm2n69vll00010t5m329c90wf-0-0"]', '120');
...
test.only('Login to the Portal and login as signcrf', async ({ page }) => {
     // Visit the portal
   await page.goto('https://qa.akyrian.com/', { timeout: 30000 });
   await page.waitForTimeout(10000);

   // Login action
   await page.fill('#email', 'signcrf@example.com');
   await page.fill('#password', 'Password!1');
   await page.click('#loginAs-btn');
...
await page.locator('[data-cy="visit-cm31qz0990dhkigpbpkpgy2p5"]').filter({ hasText: 'AUTOMATION-sign-CRF' }).click();
    ...
    // Check the Sign CRF Screen
    await expect(page.locator('[data-cy="sign-level"]')).toHaveText('Sign AUTOMATION-sign-CRF');
    await expect(page.locator('.Text__StyledText-fcSGOX.dcoJJS')).toHaveText(
      'I, AUTOMATION- SIGNCRF, the Clinical Trial Investigator certify that I have reviewed the data and can verify that it is accurate and contemporaneous'
    );
    await expect(page.locator('[data-cy="input-password"]')).toBeVisible();
    await expect(page.locator('[data-cy="btn-sign"]')).toHaveText('Sign');
