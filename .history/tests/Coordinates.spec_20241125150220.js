...
test('Login to the Portal and login as Source Capture User', async ({ page }) => {
 ...
 // Locate the first canvas element using a more specific selector
 const canvas = page.locator('.MarkupCanvas__Container-ghQqGD > .BaseCanvas__CanvasWrapper-bQdpLq');

 // Ensure the canvas is visible
 await expect(canvas).toBeVisible();
 
   // Define the coordinates for the mouse actions
   const startX = 200;  // Starting X coordinate
   const startY = 200;  // Starting Y coordinate
   const endX = 300;    // Ending X coordinate (dragging right)
   const endY = 300;    // Ending Y coordinate (dragging down)
 
   // Simulate the mouse events
   await canvas.hover();  // Ensure the canvas is focused
   await page.mouse.move(startX, startY);  // Move the mouse to the starting point
   await page.mouse.down();  // Press down to start the drag
   await page.mouse.move(endX, endY);  // Move to the ending point (dragging to create the rectangle)
   await page.mouse.up();  // Release the mouse to complete the drag
 
   // Take a screenshot after performing the action
   await page.screenshot({ path: 'canvas-drag-screenshot.png' });
