...
await page.fill('#ap_email_login', 'divyadhileccepan1415@gmail.com');
await page.locator('.a-button-input').click();
const text1= await page.locator('[class="intent-confirmation-title"]').textContent();
console.log("Looks like you are new to Amazon")
await page.goBack();

//User enters the correct email/phone number but an incorrect password
await page.fill('#ap_email_login', 'divyadhileepan1415@gmail.com');
await page.locator('.a-button-input').click();
await page.fill('#ap_password', 'Divya1408');
await page.click('#signInSubmit');

const text2= await page.locator('[class="a-alert-heading"]').textContent();
console.log("There was a problem")
console.log("Your password is incorrect")
const text3= await page.locator('[class="a-size-large"]').textContent();
console.log("Solve this puzzle to protect your account")
await page.goto('https://www.amazon.in/your-account');
//User tries to log in without entering an email/phone number or password.
await page.fill('#ap_email_login', '');
await page.locator('.a-button-input').click();
const text4 = await page.locator('[class="a-alert-content"]').first().textContent();
console.log("Enter your mobile number or email")
//User enters the correct email address but changes the case (uppercase or lowercase) of the password.
await page.fill('#ap_email_login', 'divyadhileepan1415@gmail.com');
await page.locator('.a-button-input').click();
await page.fill('#ap_password', 'DIVYA@1408');
await page.click('#signInSubmit');
const text5= await page.locator('[class="a-alert-heading"]').textContent();
console.log("There was a problem")
console.log("Your password is incorrect")
const text6= await page.locator('[class="a-size-large"]').textContent();
console.log("Solve this puzzle to protect your account")
await page.goto('https://www.amazon.in/your-account');
//User enters the password and toggles the password visibility (eye icon)
await page.fill('#ap_email_login', 'divyadhileepan1415@gmail.com');
await page.locator('.a-button-input').click();
const passwordInput = page.locator('#ap_password');
const eyeIcon = page.locator('[class="a-icon a-icon-checkbox"]');
await passwordInput.fill('Divya@1408');

  // Verify the input type is "password" initially
  await expect(passwordInput).toHaveAttribute('type', 'password');
  
  // Click on the eye icon to toggle password visibility
  await eyeIcon.click();
   // Verify the input type changes back to "password"
   await expect(passwordInput).toHaveAttribute('type', 'password');

  
  // Click on the eye icon again to toggle back
  await eyeIcon.click();
    // Verify the input type changes to "text" (password visible)
    await expect(passwordInput).toHaveAttribute('type', 'password');
    //User clicks on the "Forgot your password?" link and enters the registered email or phone number.

await page.locator('#auth-fpp-link-bottom').click();
//User enters an email address or phone number with special characters.
await page.goto('https://www.amazon.in/your-account');
...
const text7= await page.locator('[class="a-alert-content"]').nth(2).textContent();
console.log("Invalid email address.")
...
await page.fill('#ap_password', 'Divya@1408');
await page.click('#signInSubmit');
//Click on the "Your Orders" link to verify that it redirects to the order history page.
await page.locator('[class="a-box-inner"]').first().click();
await expect(page).toHaveURL('https://www.amazon.in/gp/your-account/order-history?ref_=aw_ya_hp_oh_aui', { timeout: 60000 });
// Click on the "Wishlist" link to verify that it redirects to the wishlist or saved items page.
await page.goBack();
await page.locator('[class="a-box-inner"]').nth(28).click();
await expect(page).toHaveURL('https://www.amazon.in/gp/yourstore?ref_=ya_maw_ysh', { timeout: 60000 });
await page.goBack();
await page.locator('[class="a-box-inner"]').nth(4).click();
await expect(page).toHaveURL('https://www.amazon.in/gp/aw/address-book?ref_=aw_ya_hp_manage_address', { timeout: 60000 });
await page.goBack();
//Click on the "Payment Methods" link to verify that it redirects to the payment methods page.
await page.locator('[class="a-box-inner"]').nth(21).click();
await expect(page).toHaveURL('https://www.amazon.in/cpe/managepaymentmethods?ref_=ya_mb_mpo', { timeout: 60000 });
await page.goBack()
...
const myElement = page.locator('[class="a-button-text"]');
await expect(myElement).toHaveText('Go to Cart');
await myElement.click();

// Click on 'Proceed to buy'
await page.locator('[value="Proceed to checkout"]', {timeout: 40000 }).click();
//Add Delivery Address
await page.locator('[class="a-button a-spacing-top-small a-button-primary a-button-small"]', {timeout: 40000 }).click();
await page.fill('#address-ui-widgets-enterAddressLine1', 'puthucherry street');
await page.fill('#address-ui-widgets-enterAddressLine2', 'Mappillaikkupam');
await page.fill('#address-ui-widgets-landmark', 'Near Mosque');
await page.fill('#address-ui-widgets-enterAddressPostalCode', '610105');
await page.fill('#address-ui-widgets-enterAddressCity', 'Nannailam');
