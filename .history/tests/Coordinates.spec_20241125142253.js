...
test('Login to the Portal and login as Source Capture User', async ({ page }) => {
  ...
  // Click Start Redaction (if required after clicking manual redaction)
  const canvas = page.locator('[style="cursor: crosshair;"] > .konvajs-content > canvas');
  await canvas.waitFor({ state: 'attached' });  // Ensure the canvas is attached to the DOM

// Now, get the bounding box
const rect = await canvas.boundingBox();

  // Adjust the starting position and movement as per your log
