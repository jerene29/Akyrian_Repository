...
// Optionally, scroll the element into view if necessary
await page.locator('#cy-tabsource').scrollIntoViewIfNeeded();

// Click the element (force option if necessary)
...
//await page.locator("[data-cy='capture-action-cm2n696qj08n8zdk3hgiru340'] > .question-card-action-menu-icon").click({ timeout: 60000 });
const path = require('path'); // Import the 'path' module

const locator = page.locator('[data-cy="capture-action-cm2n696qj08n8zdk3hgiru340"] > .question-card-action-menu-icon').first();

// Wait for the element to be visible, then click it
await locator.waitFor({ state: 'visible', timeout: 60000 }); // Adjust timeout as needed
await locator.click();
  const fileInput = page.locator('input[type="file"]');
  
  // Specify the path to your image file
  const imagePath = path.join(__dirname, 'Images', 'Image4.png'); // Replace with your actual image file path
  
  // Upload the image file
  await fileInput.setInputFiles(imagePath);  // Update with correct path to your image
...
const element = page.locator('[data-cy="manual-redact-button"]');
...
const locator1 = page.locator('[data-cy="capture-action-cm2n696qj08n8zdk3hgiru340"] > .question-card-action-menu-icon').first();
...
await locator.click();
  ...
  // Specify the path to your image file
  const imagePath1 = path.join(__dirname, 'Images', 'Image4.png');
...
const element1 = page.locator('[data-cy="manual-redact-button"]');
...
const locator2 = page.locator('[data-cy="capture-action-cm2n696qj08n8zdk3hgiru340"] > .question-card-action-menu-icon').first();
...
await locator.click();
  ...
  // Specify the path to your image file
  const imagePath2 = path.join(__dirname, 'Images', 'Image4.png');
// Wait for the modal and click 'Yes'
await page.waitForTimeout(30000);
await expect(page.locator('[data-cy="confirmation-modal-title"]')).toHaveText(/No Name Found/);
await page.locator('[data-cy="confirmModal-confirmButton"]').click();
...
const element2 = page.locator('[data-cy="manual-redact-button"]');
