...
test.only('Login_TC001: Check the Login button', async ({ page }) => {
    ...
    const ForgetPassword = page.locator('.ui-inputgroup-addon > [_ngcontent-kiw-c53] > [_ngcontent-kiw-c53]');
    await expect(captchaButton).toHaveText('FORGOT ACCOUNT DETAILS?');
    ...
    const OTP = page.locator('[class="css-label_c t_c"]');
    await expect(OTP).toHaveText('Login & Booking With OTP');
    const SIGNIN = page.locator('[class="search_btn train_Search"]');
    await expect(SIGNIN).toHaveText('SIGNIN');
    const REGISTER = page.locator('[class="search_btn"]');
    await expect(REGISTER).toHaveText('REGISTER');
    const AGENTLOGIN = page.locator('[class="search_btn"]');
