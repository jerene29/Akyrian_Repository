...
test.only('Login_TC001: Check the Login button', async ({ page }) => {
  ...
  await page.locator('button.search_btn.train_Search').nth(2).waitFor({ state: 'visible' });

  // Ensure no overlay or iframe is blocking the button (optional)
  //await page.locator('.ui-dialog-mask').waitFor({ state: 'hidden' });

  // Click the button
  await page.click('button.search_btn.train_Search').nth(2);
