...
test('Check the Amazon portal home page', async ({ page }) => {
  ...
  await page.waitForSelector('#auth-email-invalid-claim-alert .a-alert-content', { timeout: 10000 });
  const textContent = await page.textContent('#auth-email-invalid-claim-alert .a-alert-content');
  expect(textContent).toContain('Invalid email address.');
