...
test.only('Login to the Portal and login as Source Markup user', async ({ page }) => {
    ...
    // Search study
    await page.fill('[data-cy="onboarding-search-study"]', 'QAonCloud Test');
    await page.click('[class="Text__StyledText-fcSGOX liUSLx adjust-version mb-0.5"]:has-text("v.4.b")');
    await page.waitForTimeout(10000);

    // Select the patient
    await page.click('[class="Text__StyledText-fcSGOX gSpvOG sider-patient-name"]:has-text("QA_AU-02")');
    await page.waitForTimeout(10000);

    // Click visit
    await page.click('[data-cy="visit-cm2ol891a0amjzdk3m1xitkk2"][data-visit-name="AUTOMATION-sign-CRF"]');
    await page.waitForTimeout(10000);

    // Check Pending Snippet-1
    await page.click('[data-cy="question-card-cm2n696qj08n8zdk3hgiru340"] >> text="CRF: Allergies-AT"');
    await page.waitForTimeout(10000);
    await page.click('[class="question-card-action-menu-icon icon-dettach"]:first', { force: true });
    await page.waitForTimeout(10000);
