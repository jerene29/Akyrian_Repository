...
test('Login to the Portal and login as Source Capture User',async ({ page }) => {
    ...
    await page.locator('[data-cy="onboarding-search-study"]').waitFor({ state: 'visible' }, { timeout: 100000 });
    ...
    .filter({ hasText: 'QA_AU-02' }) // Filters for the element containing the text "QA_AU-02"
    .waitFor({ state: 'visible', timeout: 60000 }) // Waits for the element to become visible (60 seconds timeout)
    .click();

    // Wait for the visit tab and click
    await page.locator('[data-cy="visit-cm31qz0990dhkigpbpkpgy2p5"]').scrollIntoViewIfNeeded({ timeout: 60000 }); // 60 seconds timeout
...
await page.locator('[data-cy="visit-cm31qz0990dhkigpbpkpgy2p5"]')
    ...
    await page.locator('[data-cy="sourceQuestionTab"]').waitFor({ state: 'visible', timeout: 60000 }); // 60 seconds timeout
...
await page.locator('[data-cy="upload-sc-button"]').waitFor({ state: 'visible', timeout: 60000 }); // 60 seconds
