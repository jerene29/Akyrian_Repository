...
//Click View Seats
await page.locator("[boostdata='{\"preBoostPos\":1,\"isBoosted\":false,\"wtPrice\":1068.18,\"seatPrices\":\"1050.0:14,1100.0:8\",\"compRids\":\"\",\"compSids\":\"\",\"campaignId\":\"ALLC_e9b73d18\",\"boostFactor\":0,\"boostDiscPer\":9.36,\"dealType\":\"FEMALE\"}'] > .button", { timeout: 10000 }).click()
...
await page.locator(".button", { timeout: 10000 }).click()
//Click Home
await page.locator(".crumb-href", { timeout: 10000 }).click()
//Click Account
await page.locator("#account_dd .name_rb_secondary_item", { timeout: 10000 }).click()
//Click Cancel Ticket
//await page.locator("#cancel_ticket > .header_dropdown_item_name", { timeout: 10000 }).click()
const [newWindow] = await Promise.all([
    page.context().waitForEvent('popup'), // Wait for the new window
    page.click('#cancel_ticket > .header_dropdown_item_name') // Click the link that opens in a new window
  ]);

  // Now you can interact with the new window
  await newWindow.waitForLoadState(); // Wait for the new window to load

  // Example: Check the URL of the new window
  console.log(await newWindow.url()); // Logs the URL of the new window
  expect(await newWindow.url()).toBe('https://www.redbus.in/Cancellation');
