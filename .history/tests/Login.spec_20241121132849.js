...
test('Login to the Portal and login as Source Capture User',async ({ page }) => {
    ...
    await page.locator('[class="Sider__SelectablePatient-bsqmeQ lhVINm hover-all-selectablepatient"]').eq(1).waitFor({ state: 'visible' }, { timeout: 40000 });

    await page.locator('[class="Sider__SelectablePatient-bsqmeQ lhVINm hover-all-selectablepatient"]').eq(1)
