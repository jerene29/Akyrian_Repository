...
cy.dragAndDrop(
  ...
  cy.get('[data-cy="add-new-question-page"]')
    .find('[data-cy="field-card-container-12"]', { timeout: 10000 })  // Wait for the element with increased timeout
...
cy.get('.ant-tabs-nav-list > :nth-child(2)',{ timeout:30000 }).click()
  //Second Question
  cy.dragAndDrop(
    '[data-cy="field-card-7"]', 
    '.eiFHyU .ant-row',
    0 // Optional: Vertical offset if needed
  ).then(() => {
    cy.get('[data-cy="add-new-question-page"]')
    .find('[data-cy="field-card-container-14"]') 
    .should('exist') 
  });
  //cy.get('[name="attr-noSCNeeded"]').click()
  cy.get('[data-cy="short-question-input-14"').click({force: true}).type("ZZXX")
  cy.get('[data-cy="long-question-input-14"]').click( {force: true}).type("ZXZXX").invoke('text').as('Q2')
  cy.get('[data-cy="long-question-input-14"]').then(($text1) => {
    const Q2 = $text1.text()
    cy.get('[data-cy="long-question-input-14"]').should('contain', Q2)

  cy.get('[data-cy="oid-input-14"]').type("ZZXX")
  cy.get('[data-cy="keyword-question-input-14"]').click( {force: true}).type("XSZZZ")
  cy.get('[name="attr-markAsNoAnswer"]').click( {force: true})

  cy.get('[data-cy="right-menu-save-button"]',{ timeout:30000 }).click()
  ...
  cy.dragAndDrop(
    ...
    '[class="QuestionCanvas__Container-dHhNeJ hzHpYc"]',{ timeout: 50000 },
   0// Optional: Vertical offset if needed
  ).then(() => {
    cy.get('[data-cy="add-new-question-page"]')
  ...
  //cy.get('[name="attr-noSCNeeded"]').click()
  cy.get('[data-cy="short-question-input-16"',{ timeout:40000 }).click( {force: true}).type("Numeric Question")
  cy.get('[data-cy="long-question-input-16"]',{ timeout:30000 }).click( {force: true}).type("Numeric Question") .invoke('text').as('Q3')
  cy.get('[data-cy="long-question-input-16"]').then(($text1) => {
    const Q3 = $text1.text()
    cy.get('[data-cy="long-question-input-16"]').should('contain', Q3)
  cy.get('[data-cy="oid-input-16"]').type("Test2")
  cy.get('[data-cy="keyword-question-input-16"]').click( {force: true}).type("Visit_01")
  cy.get('[data-cy="number-max-digits-input-16"]').click( {force: true}).type("4")
