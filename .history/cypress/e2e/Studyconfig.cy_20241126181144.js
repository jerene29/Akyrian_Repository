...
cy.wait(20000)
 cy.get('[name="attr-noSCNeeded"]').click()
  ...
  cy.get('[data-cy="long-question-input-18"]').click( {force: true}).type("Upload").invoke('text').as('Q2')
  cy.get('[data-cy="long-question-input-18"]').then(($text1) => {
    const Q2 = $text1.text()
    cy.get('[data-cy="long-question-input-18"]').should('contain', Q21)

  cy.get('[data-cy="oid-input-18"]').type("ZZXX")
  cy.get('[data-cy="keyword-question-input-18"]').click( {force: true}).type("FileUpload")
  cy.get('[name="attr-markAsNoAnswer"]').click( {force: true})
