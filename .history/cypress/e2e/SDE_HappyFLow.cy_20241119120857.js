...
//Question 1-Check Image upload
 cy.get('[data-cy="question-card-cm2n696qj08n8zdk3hgiru340"] > [data-cy="question-card"]', { timeout: 10000 }).click()
...
cy.get('[class="Text__StyledText-fcSGOX kZdsfX mb-3"]').contains("Verification Result")
// Check that the patient name is "Not Found"
cy.get('[data-cy="not-found-data-patientsName"]').should('have.text', ': Not Found');
// Check that the date of birth is marked as "Verified"
cy.get('[data-cy="verified-data-dateOfBirth"]').should('have.text', ': Verified');
...
cy.get('[data-cy="submit-bottom-chips-menu"]').dblclick( {force: true})
cy.wait(50000)
...
cy.get('input[type="file"]', { timeout: 10000 }).attachFile(filepath2);
cy.wait(40000)
//If we upload not available DOB  it shows as DOB as Notfound
cy.get('[class="Text__StyledText-fcSGOX kZdsfX mb-3"]').contains("Verification Result")
 // Check that the DOB is "Not Found"
 cy.get('[data-cy="not-found-data-dateOfBirth"]').should('have.text', ': Not Found');
 // Check that the patientsName is marked as "Verified"
 cy.get('[data-cy="verified-data-patientsName"]').should('have.text', ': Verified');
...
cy.get('[class="Text__StyledText-fcSGOX kZdsfX mb-3"]').contains("Verification Result")
 ...
 cy.get('[data-cy="not-found-data-visitDate"]').should('have.text', ': Not Found');
 // Check that the patientsName is marked as "Verified"
 cy.get('[data-cy="verified-data-patientsName"]').should('have.text', ': Verified');
...
cy.get('[data-cy="ATTACHED"]', { timeout: 20000 }).click()
...
cy.get('[data-cy="reattach-action"]').first().click({force: true})
