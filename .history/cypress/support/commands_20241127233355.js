...
*/
Cypress.Commands.add('dragAndDrop', (subjectSelector, targetSelector, yOffset = 0) => {
  const BUTTON_INDEX = 0;
  const SLOPPY_CLICK_THRESHOLD = 10;

  // Initialize counter to track the number of drag-and-drop operations
  let dragCount = 0;

  // Function to perform drag and drop
  const performDragAndDrop = (subject, target) => {
    cy.get(target).then(($targets) => {
      const coordsDrop = $targets[0].getBoundingClientRect(); // Get the drop target's coordinates
      ...
      cy.get(subject).then(($subject) => {
        ...
        // Trigger dragover event on all target elements (iterate over each one)
        cy.wrap($targets).each(($target) => {
          cy.wrap($target).trigger('dragover', { force: true });
        });
        ...
        // Increment the counter after each drag-and-drop operation
        dragCount++;

        // Perform mousedown only after the 4th drag-and-drop operation
        if (dragCount === 3) {
          cy.get('body').trigger('mousedown', {
            force: true,
            clientX: coordsDrop.x + (coordsDrop.width / 2),
  ...
  performDragAndDrop(subjectSelector, targetSelector);
});
