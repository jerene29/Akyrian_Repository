const { test, expect } = require('@playwright/test');
    ...
    test('Login to the Portal and login as Source Capture User', async ({ page }) => {
        ...
        // Visit the portal
        await page.goto('https://qa.akyrian.com/', { waitUntil: 'networkidle' }); // Increase timeout to 60 seconds
        // Login action
...
await page.locator('[data-cy="onboarding-search-study"]', { timeout: 60000 }).click();
await page.fill('#search', 'QAonCloud Test', { timeout: 60000 });

  
// Click the study version.
await page.locator("[data-test='cm226tpiz0kxl14ev4pzlcvfc'] #study-versionmain", { timeout: 60000 }).filter({ hasText: 'v.4.b' }).click();

// Click the patient name
await page.locator('[class="Text__StyledText-fcSGOX gSpvOG sider-patient-name"]', { timeout: 60000 }).filter({ hasText: 'QA_AU-02' }).click();

// Wait for the visit tab to load and click the visit card
await page.locator('[data-cy="visit-cm2ol891a0amjzdk3m1xitkk2"]').filter({ hasText: 'AUTOMATION-sign-CRF' }).click();
